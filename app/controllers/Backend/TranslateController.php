...
class TranslateController extends AdminController {
    ...
    /**
     * Display a listing of the translates.
     * @param $module
     * @param $id
     * @return Response
     */
    ...
    {
        dd(call_user_func(array('Components\Stones\Models\\'.$module, 'all')));
        $translates = Translate::all();
        // dd($translates);
        $this->layout->title = 'All Users';
        $this->layout->content = View::make($this->link_type.'.'.$this->current_theme.'.translates.index')
    ...
    public function create()
    {
        ...
        $this->layout->content = View::make($this->link_type.'.'.$this->current_theme.'.translates.create_edit');
    }
    ...
    public function show($id)
    {
        ...
        $this->layout->content = View::make($this->link_type.'.'.$this->current_theme.'.translates.show')
                                        ->with('user', $user);
    }
    ...
    {
        if (!($this->user->hasAccess('translates.edit') || Sentry::getUser()->id == $id)) App::abort('401');

        try {
            if (current_user()->id != $id ) {
                $redirect_to = $this->link_type . '/translates';
            } else {
                // Redirect translates without user editing privilege to their profile
    ...
    public function getChangePassword()
    {
        ...
        $this->layout->content = View::make($this->link_type.'.'.$this->current_theme.'.translates.change_pw');
    }
    ...
    public function destroy($id=null)
    {
        try {
            ...
            return Redirect::to("{$this->link_type}/translates")
