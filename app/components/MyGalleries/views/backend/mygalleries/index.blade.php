            ...
            <div class="widget light-gray box">
                ...
                <div class="widget-body">
                    <div class="clearfix margin-bottom-10">
                        <div class="btn-group pull-right">
                            <div class="actions inline">
                                ...
                                <ul class="btn">
                                    @if ($current_user->hasAccess("mygalleries.destroy"))
                                    <li>
                                        {{ Form::open(array('route' => array($link_type . '.mygalleries.destroy', 'multiple'), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecords($(this), 'mygalleries');")) }}
                        ...
                        @if ($current_user->hasAccess("mygalleries.create"))
                        <div class="btn-group pull-right">
                            <button data-href="{{ URL::to($link_type . '/mygalleries/create') }}" class="btn btn-success">
                    ...
                    <table class="table table-striped table-hover table-bordered" id="sample_1">
                        ...
                        <tbody>
                        @foreach( $mygallaries as $mygallery )
                            <tr>
                                <td>{{ Form::checkbox($mygallery->id, 'checked', false) }}</td>
                                
                                <td>{{ $mygallery->name }}</td>
                                <td>{{ $mygallery->status }}</td>
                                <td>{{ $mygallery->author }}</td>
                                <td>
                                    <a href="{{ URL::to($link_type . '/mygalleries/' . $mygallery->id . '/edit') }}" class="btn btn-mini"><i class="icon-edit"></i></a>
                                    ...
                                    <div class="actions inline">
                                        ...
                                        <ul class="btn btn-mini">
                                            @if ($current_user->hasAccess("mygalleries.destroy"))
                                            <li>
                                                {{ Form::open(array('route' => array($link_type . '.mygalleries.destroy', $mygallery->id), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecord($(this), 'my gallery');")) }}
                                                    <button type="submit" class="danger delete"><i class="icon-trash"></i> Delete</button>
                                                {{ Form::close() }}
                                            </li>
                                            @endif
