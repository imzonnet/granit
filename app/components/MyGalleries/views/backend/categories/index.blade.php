                    ...
                    <div class="clearfix margin-bottom-10">
                        <div class="btn-group pull-right">
                            <div class="actions inline">
                                ...
                                <ul class="btn">
                                    @if ($current_user->hasAccess("mygallery-categories.destroy"))
                                    <li>
                                        {{ Form::open(array('route' => array($link_type . '.mygallery-categories.destroy', 'multiple'), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecords($(this), 'mygallery-categories');")) }}
                        ...
                        @if ($current_user->hasAccess("mygallery-categories.create"))
                        <div class="btn-group pull-right">
                            <button data-href="{{ URL::to($link_type . '/mygallery-categories/create') }}" class="btn btn-success">
                    ...
                    <table class="table table-striped table-hover table-bordered" id="sample_1">
                        ...
                        @foreach( $categories as $category )
                            <tr>
                                ...
                                <!-- 
                                <td>{{ HTML::link(url($link_type . '/mygallery-categories/'.$category->id), $category->name) }}</td>
                                 -->
                                <td>{{ $category->name }}</td>
                                <td>{{ $category->status() }}</td>
                                <td>{{ $category->author() }}</td>
                                <td>
                                    <a href="{{ URL::to($link_type . '/mygallery-categories/' . $category->id . '/edit') }}" class="btn btn-mini"><i class="icon-edit"></i></a>
                                        ...
                                        <div class="actions inline">
                                            ...
                                            <ul class="btn btn-mini">
                                                @if ($current_user->hasAccess("mygallery-categories.destroy"))
                                                <li>
                                                    {{ Form::open(array('route' => array($link_type . '.mygallery-categories.destroy', $category->id), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecord($(this), 'product category');")) }}
                                                        <button type="submit" class="danger delete"><i class="icon-trash"></i> Delete</button>
                                                    {{ Form::close() }}
                                                </li>
                                                @endif
