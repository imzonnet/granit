            ...
            <div class="widget-body">
                <div class="clearfix margin-bottom-10">
                    <div class="btn-group pull-right">
                        <div class="actions inline">
                            ...
                            <ul class="btn">
                                @if ($current_user->hasAccess("testimonials.destroy"))
                                <li>
                                    {{ Form::open(array('route' => array($link_type . '.testimonials.destroy', 'multiple'), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecords($(this), 'testimonials');")) }}
                    ...
                    @if ($current_user->hasAccess("testimonials.create"))
                    <div class="btn-group pull-right">
                        <button data-href="{{ URL::to($link_type . '/testimonials/create') }}" class="btn btn-success">
                ...
                <table class="table table-striped table-hover table-bordered" id="sample_1">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ Form::checkbox($testimonial->id, 'checked', false) }}</td>
                            <td>{{ $testimonial->name }}</td>
                            <td>{{ $testimonial->title }}</td>
                            <td>{{ $testimonial->description }}</td>
                            <td>{{ $testimonial->rate }}</td>
                            <td>
                                <a href="{{ URL::to($link_type . '/testimonials/' . $testimonial->id . '/edit') }}" class="btn btn-mini"><i class="icon-edit"></i></a>
                                <div class="actions inline">
                                    ...
                                    <ul class="btn btn-mini">
                                        @if ($current_user->hasAccess("testimonials.destroy"))
                                        <li>
                                            {{ Form::open(array('route' => array($link_type . '.testimonials.destroy', $testimonial->id), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecord($(this), 'product product');")) }}
                                            <button type="submit" class="danger delete"><i class="icon-trash"></i> Delete</button>
                                            {{ Form::close() }}
                                        </li>
                                        @endif
