            ...
            <div class="widget-body">
                <div class="clearfix margin-bottom-10">
                    <div class="btn-group pull-right">
                        <div class="actions inline">
                            ...
                            <ul class="btn">
                                @if ($current_user->hasAccess("memorial-media.destroy"))
                                <li>
                                    {{ Form::open(array('route' => array($link_type . '.memorial.media.destroy', $memorial->id, 'multiple'), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecords($(this), 'memorials');")) }}
                    ...
                    <div class="btn-group pull-right">
                        <button data-href="{{URL::route($link_type.'.memorial.media.create', $memorial->id)}}" class="btn btn-success">
                ...
                <table class="table table-striped table-hover table-bordered" id="sample_1">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ Form::checkbox($media->id, 'checked', false) }}</td>

                            <td>{{ HTML::linkRoute($link_type . '.memorial.media.show', $media->title, [$media->memorial_id, $media->id]) }}</td>
                            <td>{{ $media->author() }}</td>
                            <td>{{ $media->created_at }}</td>
                            <td>
                                @if ($current_user->hasAccess("memorial-media.create"))
                                <a href="{{URL::route($link_type . '.memorial.media.edit', [$media->memorial_id, $media->id])}}" class="btn btn-mini"><i class="icon-edit"></i></a>
                                @endif
                                <div class="actions inline">
                                    ...
                                    <ul class="btn btn-mini">
                                        @if ($current_user->hasAccess("memorial-media.destroy"))
                                        <li>
                                            {{ Form::open(array('route' => array($link_type . '.memorial.media.destroy',$media->memorial_id, $media->id), 'method' => 'delete', 'class'=>'inline', 'onsubmit'=>"return deleteRecord($(this), 'Memorial');")) }}
                                            <button type="submit" class="danger delete"><i class="icon-trash"></i> Delete</button>
                                            {{ Form::close() }}
                                        </li>
                                        @endif
